...
"""
Programme 1 
Objectif : Prédire les valeurs de la variable cible 'V200'.
"""
...
def main():
    ...
    # ---------- Prédiction ----------
    Predictions = dtree.predict(deploiement.loc[:,selector.get_support()])
    ...
    print("--> Le fichier 'predictions.txt' est dans votre répertoire courant : ", chemin)
...
fenetre = Tk()
fenetre.title("Programme 1 - Data Mining")
...
# Grand titre 
Indic_crit = Label(fenetre, text="Programme 1 - Data Mining \n",fg = "darkblue", bg='skyblue', font = ("Helvetica", 20 , "bold"))
Indic_crit.grid(row=0, column=0, columnspan=2)

##### Objectif
label_objectif = Label(fenetre, justify=LEFT, text="Objectif : Prédire les valeurs de la variable cible 'V200'. \n",fg = "DodgerBlue4", bg='skyblue', font = ("Arial", 18 , "normal"))
...
#### Méthode
label_methode = Label(fenetre, justify=LEFT, text="Avec la méthode de prédiction Arbre de Décision, l'estimation du nombre erreur de classement sur \n une base de 4 898 424 observations est de 1547. \n",fg = "DodgerBlue4", bg='skyblue', font = ("Arial", 18 , "normal"))
label_methode.grid(row=5, column=0, columnspan=2, sticky="w")

#### Information sur le nouveau fichier créé
label_info = Label(fenetre, text="Le fichier \"predictions.txt\" sera disponible dans votre répertoire courant à la fin du \n traitement.\n",
